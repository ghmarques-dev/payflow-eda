...
export class SaleCheckoutRequestedHandlerService implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    await this.eventSubscriber.subscribe({
      ...
      handler: (payload) =>
        this.handle(payload as SaleCheckoutRequestedEventPayload),
    });
  }

  private async handle(
    payload: SaleCheckoutRequestedEventPayload,
  ): Promise<void> {
    await tracer.startActiveSpan(
      "SaleCheckoutRequestedHandler.handle",
      async (root) => {
        ...
        try {
          ...
          for (const item of items) {
            await tracer.startActiveSpan(
              "SaleCheckoutRequestedHandler.reserve_line_item",
              async (span) => {
